...
fs.readdir('./src/commands/', (err, files) => {
  ...
  console.log(`[ahmet] ${files.length} komut yüklenecek.`);
  files.forEach(f => {
    fs.readdir("./src/commands/" + f, (err2, files2) => {
      files2.forEach(file => {
        let props = require(`./src/commands/${f}/` + file);
        console.log(`[ahmet KOMUT] ${props.conf.name} komutu yüklendi!`);
...
client
  ...
  (async () => {
    try {
      console.log('[ahmet] Slash Komutlar yükleniyor.');
      await rest.put(
        Routes.applicationCommands(process.env.BotClientID),
        { body: slashcommands },
      );
      console.log('[ahmet] Slash Komutlar yüklendi.');
    } catch (error) {
      console.error(error);
    }
  })();
