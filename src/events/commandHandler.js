...
module.exports = async (message) => {
  ...
  const embed = new MessageEmbed().setFooter({ text: `Developer By ahmet`}).setColor(message.member.displayHexColor).setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true })});
  ...
  if (cmd) {
    ...
    `)
    if(komutLog) komutLog.wsend({ embeds: [ahmet]})
  }
};
