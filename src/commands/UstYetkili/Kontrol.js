...
.setThumbnail(message.author.displayAvatarURL({ dynamic: true, size: 2048 }))
  ...
  let msg = await message.channel.send({ embeds: [ahmet], components: [row]})
    ...
    let ahmet = message.guild.members.cache.filter(member => !member.roles.cache.has(etkinlik) || !member.roles.cache.has(cekilis))
    button.reply({ content:`
Etkinlik/Çekiliş rolü olmayan ${ahmet.size} kullanıcıya etkinlik, çekiliş rolleri verildi !`})
    ...
    if (button.customId === 'tagrol') {
      ...
      let taglilars = message.guild.members.cache.filter(s => s.user.username.includes(ayar.tag) && !s.roles.cache.has(ayar.ekipRolu))
...
${taglilar.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")}`})
    ...
    button.reply({ content:`
Kayıtsız rolü olmayan ${ahmetcim.size} kullanıcıya kayıtsız rolü verildi !

Kayıtsız Rolü verilen kullanıcılar;
${ahmetcim.map(x => x || "Rolü olmayan Kullanıcı bulunmamaktadır.")} `})
