...
module.exports = {
  ...
  if (!message.member.permissions.has(8n)) return message.react(red)
    ...
    if (!sec) {
      ...
      let ahmet = message.guild.members.cache.filter(yetkili => yetkili.roles.cache.has(ayar.registerPerm)).filter(yetkilises => !yetkilises.voice.channel  && yetkilises.presence && yetkilises.presence.status != "offline")
      ...
      message.channel.send({ content: `\`\`\`js\n${table.create(tablo)}\`\`\`\n\`\`\`\n${ahmet.map(yetkili => `${yetkili}`).join(', ')}\n\`\`\``})
    }
