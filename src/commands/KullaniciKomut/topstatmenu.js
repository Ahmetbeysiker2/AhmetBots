...
run: async (client, message, args, embed, prefix) => {
    ...
    const row = new MessageActionRow().addComponents(
     ...
     .setDescription(`
${client.emojis.cache.find(x => x.name === "ahmetlink")} Merhaba ${message.author}, aşağıda listelenmiş verileri inceleyip sunucunun mesaj istatistiklerini öğrenebilirsin.
    `)
    .addField(`${client.emojis.cache.find(x => x.name === "ahmetstat")} **Mesaj Kanallarının İstatistikleri**`,`${messageUsers.length > 0 ? messageUsers : "Veri Bulunmuyor."}`,true)
     ...
     .setThumbnail(message.guild.iconURL({ dynamic: true }))
     .setDescription(`
${client.emojis.cache.find(x => x.name === "ahmetlink")} Merhaba ${message.author}, aşağıda listelenmiş verileri inceleyip sunucunun ses istatistiklerini öğrenebilirsin.
    `)
    .addField(`${client.emojis.cache.find(x => x.name === "ahmetstat")} **Ses Kanallarının İstatistikleri**`,`${voiceUsers.length > 0 ? voiceUsers : "Veri Bulunmuyor."}`,true)
...
${client.emojis.cache.find(x => x.name === "ahmetlink")} Merhaba ${message.author}, aşağıda listelenmiş verileri inceleyip sunucunun ses istatistiklerini öğrenebilirsin.
    `)
    .addField(`${client.emojis.cache.find(x => x.name === "ahmetstat")} **Mesaj Kanallarının İstatistikleri**`,`${voiceUsers.length > 0 ? voiceUsers : "Veri Bulunmuyor."}`,true)
