...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    \`${index + 1}.\` (<@${m.userID}>)`).join("\n") : "Databasede Kayıtlı Davet Ettiğin Kullanıcı Bulunamadı.!")] }).then(e => setTimeout(() => e.delete().catch(() => { }), 15000)), message.react(`${client.emojis.cache.find(x => x.name === "ahmet_yes")}`)
