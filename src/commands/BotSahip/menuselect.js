...
const { ahmetnitro, ahmetnetflix, ahmetspotify, ahmetexxen, ahmetblutv, ahmetyoutube, star } = require("../../configs/emojis.json")
...
let ahmet = await message.channel.send({
    "embeds": [embed],
      "components":[{
        "type":1,
        "components":[
    ...
    let collector = await ahmet.createMessageComponentCollector({ filter,  time: 30000 })
    
    collector.on("collect", async (button) => {
    
        if (button.customId === "hepsi") {
        await ahmet.delete({ timeout: 1500 });
...
\` ⦁ \` Eğer \`@Çekiliş Katılımcısı\` Rolünü alırsanız sunucumuzda sıkça vereceğimiz ${ahmetnitro} , ${ahmetspotify} , ${ahmetyoutube} , ${ahmetnetflix} , ${ahmetexxen} ve daha nice ödüllerin bulunduğu çekilişlerden haberdar olabilirsiniz. 
