...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const ahmetcik = await message.reply({ content: 
        `${members
            .map((member, idx) => `**${idx + 1}. ${member.toString()}**`)
            .join("\n")}\nBu üyeleri banlamak istiyor musun?` })

    await ahmetcik.react(green);

    const filter = (reaction, user) => {
        return reaction.emoji.name === 'green' && user.id === message.author.id;
    };
    const collector = ahmetcik.createReactionCollector({ filter, time: 10000 });

    collector.on("collect", async () => {
        await ahmetcik.edit({ content: `${green} ${members.length} adet kullanıcı başarıyla yasaklandı.`})
    ...
    collector.on("end", (_, reason) => {
        if (reason === "time")
            ahmetcik.edit({ content: `10 saniye geçtiği için işlem iptal edildi.`})
    });
