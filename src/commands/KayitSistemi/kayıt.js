...
run: async (client, message, args, embed, prefix) => { 
  ...
  let ayar = await serverSettings.findOne({
    ...
    {
    message.react(red)
    message.reply({ content:`\`.kayıt <@ahmet/ID> <Isim> <Yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
...
if(button.customId === "MAN") {
  ...
  if(msg) msg.delete();
  button.reply({ embeds: [ahmete], components: [], ephemeral: true});
...
if(msg) msg.delete();
button.reply({ embeds: [ahmetk], components: [], ephemeral: true});
